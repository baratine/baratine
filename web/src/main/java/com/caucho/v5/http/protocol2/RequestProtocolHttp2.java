 ...
 */
public class RequestProtocolHttp2 implements InHttpHandler, ConnectionProtocol
{
  ...
  public RequestProtocolHttp2(ProtocolHttp httpProtocol,
                              HttpContainer httpContainer,
                              ConnectionTcp connTcp)
  {
    Objects.requireNonNull(httpProtocol);
    ...
    _conn = new ConnectionHttp2(this, PeerHttp.SERVER);
  }
  
  OutHttp getOut()
  {
    return _conn.getOutHttp();
  }
  ...
  public void onStart() throws IOException
  {
    ...
    //_conn.init(_connTcp.getReadStream(), _connTcp.writeStream());
    
    InHttp inHttp = _conn.getInHttp();
    OutHttp outHttp = _conn.getOutHttp();
    
    inHttp.readSettings();
    outHttp.updateSettings(inHttp.getPeerSettings());
  ...
  {
    return getConnection().socket();
  }

  @Override
  public StateConnection service() throws IOException
  {
    if (! _conn.getInHttp().onDataAvailable()) {
